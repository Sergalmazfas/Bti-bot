...
def get_cadastral_data(cadastral_number: str) -> dict | None:
    """Получить данные объекта по кадастровому номеру из Росреестра.
    Пытаемся несколько публичных эндпоинтов, возвращаем нормализованные поля или None.
    """
    try:
        headers = {
            "Accept": "application/json, text/plain, */*",
            "User-Agent": "Mozilla/5.0 (compatible; BTI-Bot/1.0)",
        }
        # Попытка 1: старый публичный эндпоинт Росреестра
        url1 = f"https://rosreestr.gov.ru/api/online/fir_objects?cadNumber={cadastral_number}"
        r1 = requests.get(url1, headers=headers, timeout=15)
        if r1.status_code == 200:
            data = r1.json()
            # Ответ может быть списком объектов или объектом
            if isinstance(data, list) and len(data) > 0:
                ...
                obj = None
            if obj:
                # Нормализуем ключевые поля, многие поля в Росреестре вложены
                norm = {
                    "address": obj.get("objectData", {}).get("objectAddress", {}).get("readableAddress")
                        or obj.get("objectAddress") or obj.get("address"),
                    "area": obj.get("params", {}).get("area") or obj.get("area"),
                    "purpose": obj.get("params", {}).get("purpose") or obj.get("utilByDoc"),
                    "build_year": obj.get("params", {}).get("yearBuilt") or obj.get("yearBuilt"),
                    "cadastral_cost": obj.get("cadCost") or obj.get("cadPrice") or obj.get("cadastralCost"),
                }
                # Очистка площади/стоимости (могут быть строками)
                try:
                    if isinstance(norm["area"], str):
                        norm["area"] = float(norm["area"].replace(',', '.').split()[0])
                except Exception:
                    pass
                try:
                    if isinstance(norm["cadastral_cost"], str):
                        norm["cadastral_cost"] = float(norm["cadastral_cost"].replace(' ', '').replace(',', '.'))
                except Exception:
                    pass
                return norm
        # Можно добавить альтернативный сервис (ROSSINDEX) при необходимости
        return None
    except Exception as e:
        logger.error(f"Rosreestr API error: {e}")
        return None

async def send_bti_card(update: Update, cadastral_number: str, info: dict) -> None:
    """Сформировать и отправить карточку БТИ с официальными данными."""
    address = info.get("address") or "—"
    area = info.get("area")
    area_str = f"{area} м²" if area else "—"
    purpose = info.get("purpose") or "—"
    year = info.get("build_year") or "—"
    cost = info.get("cadastral_cost")
    cost_str = f"{cost:,.0f} ₽".replace(",", " ") if isinstance(cost, (int, float)) else "—"
    message = (
        "🏛️ БТИ данные:\n\n"
        f"📍 Адрес: {address}\n"
        f"📐 Площадь: {area_str}\n"
        f"🏷️ Назначение: {purpose}\n"
        f"📅 Год постройки: {year}\n"
        f"💰 Кадастровая стоимость: {cost_str}\n"
    )
...
async def process_cadastral_number(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Сначала пытаемся получить официальные данные Росреестра
    await update.message.reply_text("🔎 Запрашиваем официальные данные Росреестра...")
    official = get_cadastral_data(cadastral_number)
    if official:
        # Отправляем карточку и переводим на выбор типа помещения
        await send_bti_card(update, cadastral_number, official)
        user_data[user_id].update({
            'address': official.get('address'),
            'area': official.get('area'),
            'build_year': official.get('build_year'),
            'price_online': None,
            'cadastral_cost': official.get('cadastral_cost'),
            'search_type': 'cadastral'
        })
        # Предлагаем продолжить расчёт
        keyboard = [
            [InlineKeyboardButton("🏠 Жилое", callback_data="room_type_жилое")],
            [InlineKeyboardButton("🏢 Другое", callback_data="room_type_другое")]
        ]
        await update.message.reply_text("🏠 Выберите тип помещения:", reply_markup=InlineKeyboardMarkup(keyboard))
        user_data[user_id]['step'] = 'waiting_room_type'
        return
    
    # Если Росреестр не дал данных — fallback на текущий SerpRiver-поиск
    await update.message.reply_text("↪️ Официальные данные недоступны. Пытаемся найти информацию через поиск...")
    ...
    if api_data and api_data.get('res'):
        ...
        logger.warning(f"Cadastre not found for {cadastral_number}")
        # Оставляем шаг на ожидании кадастрового номера, чтобы пользователь мог повторить ввод
...
def main():
    """Инициализация Telegram Application и хэндлеров (без запуска polling)."""
    global application, db
    # Инициализация базы данных (если не инициализирована)
    db = EnhancedDatabase()
    # Инициализация бота
...
async def _async_initialize_and_start() -> None:
    global application
    await application.initialize()
    await application.start()

async def _async_process(update_json) -> None:
    global application
    update = Update.de_json(update_json, application.bot)
    await application.process_update(update)

def _ensure_loop_and_app_initialized(update_json) -> None:
    global application, application_initialized, event_loop, loop_thread, db
    # Ensure DB ready
    db = EnhancedDatabase()
    # Ensure single background loop
    if event_loop is None:
        new_loop = asyncio.new_event_loop()
        thread = threading.Thread(target=_start_background_loop, args=(new_loop,), daemon=True)
        thread.start()
        event_loop = new_loop
        loop_thread = thread
        logger.info("✅ Background asyncio loop started")
    # Lazy-create Application and handlers
    if application is None:
        logger.info("🚀 Инициализация BTI Bot при первом запросе...")
        # Инициализируем Application и регистрируем обработчики здесь, без вызова main()
        application = Application.builder().token(os.environ.get('BOT_TOKEN')).build()
        application.add_handler(CommandHandler('start', start))
        application.add_handler(MessageHandler(filters.TEXT & ~filters.COMMAND, handle_message))
        application.add_handler(CallbackQueryHandler(process_search_type))
        logger.info('�� BTI Bot запущен!')
    # Initialize/start Application exactly once on the background loop
    if not application_initialized:
        fut = asyncio.run_coroutine_threadsafe(_async_initialize_and_start(), event_loop)
        fut.result(timeout=30)
        application_initialized = True
        logger.info("✅ Telegram Application initialized and started")
    # Process the update on the background loop
    fut = asyncio.run_coroutine_threadsafe(_async_process(update_json), event_loop)
...
def webhook():
    ...
    try:
        ...
        _ensure_loop_and_app_initialized(update_json)
