...
def search_competitor_prices(address: str, area: float) -> list:
    """Шаг 3: Поиск цен конкурентов через SERP API (синхронно)"""
    try:
        ...
        # Формируем запросы для поиска цен
        queries = [
        ...
        logger.info(f"Found competitor prices: {all_prices}")
        return all_prices
...
def parse_competitor_prices(results: list) -> list:
    """Парсинг цен из результатов поиска"""
    prices = []
    
    for result in results:
        snippet = result.get('snippet', '').lower()
        
        # Ищем цены в рублях за м²
        price_patterns = [
            r'(\d+)\s*руб[./]?\s*м[²2]',
            r'(\d+)\s*руб[./]?\s*кв[./]?м',
        ...
        for pattern in price_patterns:
            ...
            for match in matches:
                try:
                    price = int(match)
                    if 20 <= price <= 500:  # Разумный диапазон
                        prices.append(price)
                except ValueError:
                    continue
...
def calculate_competitor_prices(competitor_prices: list) -> dict:
    """Шаг 3: Расчет карточки конкурентов"""
    if not competitor_prices:
        median_price = 150  # Дефолтная цена
    else:
        median_price = statistics.median(competitor_prices)
    
    # Применяем НДС (22%) и прибыль (10%)
    final_price = median_price * 1.22 * 1.10
...
**Данные верны?**"""
                ...
                sent_message = await update.message.reply_text(message, reply_markup=reply_markup, parse_mode=constants.Markdown)
...
• Техпаспорт: {CRPTI_COEFFICIENTS['coefficient_tech']} руб/м²"""
            ...
            await query.message.reply_text(message, reply_markup=reply_markup, parse_mode=constants.Markdown)
